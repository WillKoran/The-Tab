...
interface TabRow {
  id: string; name: string; table_label: string | null;
...
function TabScreen() {
  ...
  async function load() {
    const [{ data: t }, { data: g }, { data: i }, { data: r }, { data: u }] = await Promise.all([
      ...
      supabase.from("tab_guests").select("*").eq("tab_id", id).order("created_at"),
      supabase.from("tab_items").select("*, tab_item_assignments(guest_id)").eq("tab_id", id).order("created_at"),
      supabase.from("regulars").select("*").order("name"),
      supabase.auth.getUser(),
    ]);
    setTab(t as any);
    setGuests((g as any) || []);
    setItems(((i as any) || []).map((x: any) => ({
      id: x.id, name: x.name, price: Number(x.price), quantity: x.quantity, category: x.category,
      guest_ids: (x.tab_item_assignments || []).map((a: any) => a.guest_id),
    })));
    setRegulars((r as any) || []);
    if (u.user) {
      const { data: p } = await supabase.from("profiles").select("venmo_handle").eq("id", u.user.id).maybeSingle();
  ...
  const totals = useMemo(() => tab
    ? computeTotals(items, guests,
        { value: Number(tab.tax_value), isPercent: tab.tax_is_percent },
        { value: Number(tab.tip_value), isPercent: tab.tip_is_percent })
    : null, [items, guests, tab]);

  async function addGuest(name: string, venmo?: string) {
    if (!name.trim()) return;
    const { error } = await supabase.from("tab_guests").insert({
      tab_id: id, name: name.trim(), is_you: false, venmo_handle: venmo || "",
    });
  ...
  async function addItem(name: string, price: number, quantity: number, category: string, guestIds: string[]) {
    const { data, error } = await supabase.from("tab_items").insert({
      tab_id: id, name, price, quantity, category,
    }).select().single();
    if (error || !data) return toast.error(error?.message || "Failed");
    if (guestIds.length) {
      await supabase.from("tab_item_assignments").insert(guestIds.map(gid => ({ item_id: data.id, guest_id: gid })));
    }
    load();
  }
  ...
  async function toggleAssign(itemId: string, guestId: string) {
    ...
    if (item.guest_ids.includes(guestId)) {
      await supabase.from("tab_item_assignments").delete().eq("item_id", itemId).eq("guest_id", guestId);
    } else {
  ...
  if (loading || !tab || !totals) {
    return <div className="min-h-screen flex items-center justify-center text-brown mono text-sm">loading…</div>;
  }
  ...
  const groups: [string, Item[]][] = [["Food", foodItems], ["Drinks", drinkItems]];
  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="sticky top-0 z-30 bg-paper/95 backdrop-blur border-b border-ink/10 px-4 py-3 flex items-center justify-between">
        <Link to="/home" className="flex items-center gap-1 text-[0.7rem] tracking-[0.2em] uppercase font-bold text-ink">
          <ChevronLeft size={16} /> Home
        </Link>
        <button onClick={deleteTab} className="text-brown text-xs tracking-[0.16em] uppercase">Delete</button>
      </div>
      ...
      <header className="bg-ink text-paper px-6 pt-8 pb-10 flex items-start justify-between">
        ...
        <div className="text-right">
          ...
          />
          <div className="text-[0.68rem] tracking-[0.2em] text-paper/60 mono mt-1">{dateLabel}, {timeLabel}</div>
      ...
      <main className="px-4 -mt-6 space-y-6 pb-40">
        ...
        <div className="paper-fold overflow-hidden">
          ...
          <div className="flex">
            ...
            <div className="flex-1 px-5 py-5">
              ...
              <div>
                {guests.length === 0 && <p className="text-brown text-sm py-2">Add yourself and your friends.</p>}
                {guests.map((g, idx) => (
                  <div key={g.id}>
                    <div className="flex items-center gap-2 py-2.5">
                      <Check size={14} strokeWidth={3} className="text-ink shrink-0" />
                      <span className="font-semibold text-ink">{g.name}</span>
                      {g.is_you && <span className="stamp stamp-solid text-[0.55rem] py-[2px] px-1.5">You</span>}
                      <span className="ml-auto mono font-bold text-burnt">{money(totals.perGuestTotal[g.id] ?? 0)}</span>
                      {!g.is_you && (
                        <button onClick={() => removeGuest(g.id)} className="text-brown/70 hover:text-burnt ml-2">
                          <X size={14} />
                        </button>
                      )}
                    </div>
        ...
        <div className="paper-fold px-5 py-5">
          ...
          {groups.map(([label, list], gi) => list.length > 0 && (
            <div key={label} className={gi > 0 ? "mt-4 pt-4 border-t border-ink/10" : ""}>
              <div className="text-[0.6rem] tracking-[0.24em] uppercase text-brown font-bold mb-2">{label}</div>
              {list.map((it, idx) => (
                <div key={it.id}>
        ...
        <div className="bg-burnt text-paper px-5 py-5 flex items-center justify-between">
          <div>
            <div className="text-[0.62rem] tracking-[0.28em] uppercase text-paper/80 font-bold">Grand Total</div>
        ...
        <div className="bg-ink text-paper px-5 py-6">
          <div className="flex items-baseline justify-between">
            ...
            <span className="text-[0.6rem] tracking-[0.22em] text-paper/60 uppercase">After Tax &amp; Tip</span>
          </div>
          ...
          {guests.map((g, idx) => (
            <div key={g.id}>
              <div className="flex items-center py-3">
                ...
                {g.is_you && <span className="stamp stamp-ink ml-2 text-[0.55rem] py-[2px] px-1.5">You</span>}
                <span className="ml-auto mono font-bold text-burnt text-lg">{money(totals.perGuestTotal[g.id] ?? 0)}</span>
              </div>
        ...
        <p className="text-[0.68rem] text-brown text-center leading-snug">
          The Tab never holds or moves your money — we just open Venmo with the amount ready to send.
        </p>
      </main>
...
function TaxTipRow({ label, value, isPercent, onValue, onToggle }: {
...
function AddGuestSheet({ onClose, onAdd, regulars }: {
  onClose: () => void; onAdd: (name: string, venmo?: string) => void;
...
function AddItemSheet({ onClose, onAdd, guests }: {
  onClose: () => void;
  onAdd: (name: string, price: number, quantity: number, category: string, guestIds: string[]) => void;
  guests: Guest[];
}) {
  const [name, setName] = useState("");
    ...
    <Sheet onClose={onClose}>
      <div className="px-5 py-5">
        ...
        <form onSubmit={submit} className="space-y-3">
          <label className="block">
            <div className="text-[0.62rem] tracking-[0.22em] uppercase font-bold text-brown mb-1">Item</div>
            <input value={name} onChange={e => setName(e.target.value)} required autoFocus placeholder="Rigatoni alla vodka" />
          </label>
          <div className="grid grid-cols-2 gap-3">
            <label className="block">
              <div className="text-[0.62rem] tracking-[0.22em] uppercase font-bold text-brown mb-1">Qty</div>
              <input type="number" min="1" value={quantity} onChange={e => setQuantity(e.target.value)} className="mono" />
            </label>
            <label className="block">
              <div className="text-[0.62rem] tracking-[0.22em] uppercase font-bold text-brown mb-1">Price</div>
              <input type="number" min="0" step="0.01" value={price} onChange={e => setPrice(e.target.value)} required className="mono" placeholder="24.00" />
            </label>
          </div>
          <div>
            <div className="text-[0.62rem] tracking-[0.22em] uppercase font-bold text-brown mb-1">Category</div>
            <div className="flex gap-2">
              {(["food", "drinks"] as const).map(c => (
                <button type="button" key={c} onClick={() => setCategory(c)}
                  className={`flex-1 py-2 text-[0.7rem] tracking-[0.2em] uppercase font-bold border ${category === c ? "bg-ink text-paper border-ink" : "text-ink border-ink/25"}`}>{c}</button>
              ))}
            </div>
          </div>
          <div>
            <div className="text-[0.62rem] tracking-[0.22em] uppercase font-bold text-brown mb-1">Split between</div>
...
function SettleSheet({ onClose, onSettle, hostVenmo, guests, perGuest, tabName }: {
  onClose: () => void; onSettle: () => void; hostVenmo: string;
  guests: Guest[]; perGuest: Record<string, number>; tabName: string;
}) {
  ...
  return (
    <Sheet onClose={onClose}>
      <div className="px-5 py-5">
        ...
        {!hostVenmo ? (
          <p className="text-sm text-brown mb-4">Add your Venmo handle in Account to generate request links.</p>
        ) : (
          <p className="text-sm text-brown mb-4">Each friend can tap Request to open Venmo pre-filled with what they owe you at <span className="mono">@{hostVenmo}</span>.</p>
        )}
          ...
          {others.map(g => {
            ...
            return (
              <div key={g.id} className="flex items-center gap-3 py-2 border-b border-ink/10">
                ...
                {hostVenmo ? (
                  <a href={link} className="btn-ink text-xs py-2 px-3">Request</a>
                ) : (
        ...
        <button onClick={onSettle} className="btn-burnt w-full">Mark as settled</button>
